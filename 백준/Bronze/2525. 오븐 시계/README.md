...
### 접근 방법
2884. 알람시계 문제와 거의 유사하다.

### 풀이
알람시계 문제는 -45분이 고정이었지만,   
이 문제는 추가되는 시간이 정해지지 않았기 때문에   
출력되는 시간이 24시가 넘어갈 수도 있다.   
따라서 h == 24일 경우 h == 0으로 바꿔주는 것이 아닌   
h >= 24일 때 h == 0으로 바꿔주는 것으로 수정 필요. 
