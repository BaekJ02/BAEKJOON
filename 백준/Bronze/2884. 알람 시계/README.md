...
### 출력 
 ...
 ### 접근 방법
 m - 45 를 하고 어떻게 계산해야 해결될까 고민했지만
 h * 60 + m 을 하고 거기에 -45를 해주고 그것을 다시 60으로 나누고 몫은 시간, 나머지는 시간으로 풀면 해결된다.

 ### 풀이
 입력 시간이 0시 일 경우, 0 -> 24로 바꿔줘야 되고  
 출력 시간이 24시 일 경우, 24 -> 0으로 바꿔줘야 된다.  

### 정리
살짝 생각하는데 시간이 걸렸지만 해결 방법만 알면 어렵지 않은 문제
